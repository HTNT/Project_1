...
export const Header = () => {
    ...
    useEffect(() => {
        async function getUserMessage() {
            try {
                const conversations =
                    await conversationAPI.getConversationByUserId(
                        tokenUser,
                        user._id
                    );
    ...
    useEffect(() => {
        ...
        async function getUserMessage() {
            try {
                const conversation =
                    await conversationAPI.getConversationBy2UserId(
                        tokenUser,
                        user._id,
                        data.openmessbox
                    );
    ...
    return (
        <div className="header">
            <div className="header__logo ">
                <Link to="/my-profile">{/* <img src={logo} alt="" /> */}</Link>
                <div className="header__logo-timezone">
                    <Chip size="small" variant="shape" iconPosition="none">
                        <Filter />
                    </Chip>
                </div>
            </div>
                ...
                <div className="header__menu-message">
                    ...
                    <div className={`header__menu-message_${isOpenMessage}`}>
                        <div className="conversation-main">
                            <span className="conversation-main_title">
                                Tin Nhắn
                            </span>
                            <div className="conversation-main_filter">
                                <div className="conversation-main_filter-icon">
                                    <IoIosSearch />
                                </div>

                                <input
                                    className="conversation-main_filter-input"
                                    type="text"
                                    placeholder="Tìm đoạn tin nhắn"
                                ></input>
                            </div>
            ...
            <div
                className={`header__message-${isOpenMessageItem}`}
                style={{ border: "1px solid hidden", borderRadius: "10px" }}
            >
                {messageData && (
                    <UserConversation
                        data={messageData}
                        isOpen={isOpenMessageItem}
                        setIsOpen={setIsOpenMessageItem}
                    ></UserConversation>
                )}
            </div>
        </div>
